...
def build_query(
    ...
    - expressions are searched in title+abstract (via all:)
    - categories restrict to arXiv subject classes, e.g. cs.CL, cs.AI
    """
    expressions_part = " OR ".join([f'all:"{e}"' for e in expressions])

    if categories:
        cat_part = " OR ".join([f"cat:{c}" for c in categories])
        return f"({expressions_part}) AND ({cat_part})"

    return f"({expressions_part})"
...
def build_search_query(config: Dict) -> str:
    """Build arXiv query from config expressions and categories."""
    expressions = config.get("expressions", [])
    categories = config.get("ArXiv_categories", [])

    query = build_query(expressions=expressions, categories=categories)
